...
# Files generated by this entrypoint may contain private keys or Vault credentials.
umask 077

# Preserve Docker command overrides without requiring master configuration.
if [ "$#" -gt 0 ]; then
  exec "$@"
fi

# Set defaults before using these variables with `set -u`.
SALT_MASTER_PORT=${SALT_MASTER_PORT:-4506}
SALT_PUBLISH_PORT=${SALT_PUBLISH_PORT:-4505}
...
EOF
  ...
  # This file is owned by the entrypoint. Do not leave an earlier FIPS
  # configuration active when FIPS is explicitly disabled on restart.
  rm -f "$FIPS_CONFIG"
fi
...
# Also protect files that already existed with less restrictive permissions.
